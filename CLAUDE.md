...
依存関係はPoetryで管理し、`dev` ターゲットのDockerコンテナ内で実行する。ローカルvenvのワークフローは用意されていない — コンテナを使うこと。

**コンテナのPythonマイナーバージョンは固定されていない点に注意**: ベースイメージ `dhi.io/python:3(-debian-dev)` は浮動タグをdigestで固定しているだけなので、Renovateがdigestを更新するとマイナーバージョンごと変わりうる。実際、`pyproject.toml` の制約は `python = "^3.13"` だが、**CIのカバレッジ出力では Python 3.14.7 で動いていることが確認されている**(PR #19時点のdigest)。現在のdigestでのバージョンは次のPRのカバレッジコメント(`coverage: platform linux, python X.Y.Z`)で確認できる。なお Ruff の `target-version` は `py313` のままなので、**実行環境とlintのターゲットにずれがある**(3.13向けの構文チェックのまま3.14で動いている)。壊れてはいないが、追随させるかどうかは判断が要る。
...
- `test.yaml` — `main` へのPRで実行(`workflow_dispatch` でも手動実行可)。`dev` ターゲットのDockerイメージをビルドし、その中で `ruff check .` / `ruff format --check .` / (`tests/` があれば)`pytest --cov` を実行する。カバレッジは**閾値によるCIの失敗は設けず**、`term-missing` の標準出力をそのまま `<!-- pytest-coverage-report -->` マーカー付きのPRコメントとして投稿する(Docker Scoutと同じく再実行時は上書き。計測対象は `app/pyproject.toml` の `[tool.coverage.run]` に書いてあるため、CI側は `--cov` を値なしで渡すだけでよい)。続けて `prd` ターゲットもpushせずローカルビルドし、Docker Hub OIDCでログインした上で Docker Scout(`docker scout cves`)による脆弱性スキャンを行う。結果はcritical/high のみに絞った上でそのPRへ固定マーカー(`<!-- docker-scout-report -->`)付きコメントとして投稿し、再実行時は新規コメントを増やさず上書きする(medium/lowを含む全件は `docker-scout-report-pr-<PR番号>` という名前のArtifactとして90日保持)。この脆弱性スキャンは意図的に `main` マージ前に置いている — マージ後(=Docker Hub公開後)に気づくのではなく、公開前に気づけるようにするため。`workflow_dispatch` での手動実行時は `context.issue.number` が無いためPRコメントはスキップし、結果はジョブの実行サマリー(`core.summary`)にのみ出力する。**移行前は `ruff format` を `--check` 無しで実行していたため、整形されていないコードがあっても常に成功する無意味なステップになっていた**(移行時に修正済み)。
  - **テスト実行ステップの書き方に注意(重要・ハマりどころ)**: `tests/` の有無を見るガードは `[ -d tests ] && pytest || echo ...` ではなく `if [ -d tests ]; then ... else ... fi` で書くこと。前者は**pytestが失敗したときに `||` 側の echo が成功してステップ全体が exit 0 になる**ため、落ちるべきCIが緑になる(テストが1件も無かった頃は無害だったが、テスト追加以降は致命的)。同じ理由で、標準出力を `tee` でファイルへ落とす都合上 `shell: bash` の明示も必須 — GitHub Actionsのデフォルトシェルは `bash -e` で `pipefail` が付かないため、`shell: bash`(= `-eo pipefail`)にしないとパイプ左側のpytestの失敗が握り潰される。**なお兄弟リポジトリの `test.yaml` は前者の書き方のままなので、揃えるなら同じ修正が要る**。
...
- **Ruff/mypy/pytestの設定**: `app/pyproject.toml`。兄弟リポジトリと同一の内容に揃えている。Ruffは `select = ["ALL"]` + 広範な `ignore` リストではなく `select = ["B", "E", "F", "I", "N", "W", "C90", "PL", "RUF", "UP"]` という絞り込んだルールセット(line-length 119、`target-version = "py313"`)。mypyは `disallow_untyped_defs` / `warn_return_any` などを有効にした比較的厳格な設定。pytestは `testpaths = ["tests"]` / `pythonpath = ["."]`。カバレッジの計測対象は `[tool.coverage.run]`(`source = ["."]` / `omit = ["tests/*"]`)に書いてあるため、CI側は `--cov` を値なしで渡すだけでよい。**設定を導入した当初は ruff/mypy 自体のバージョンを兄弟と揃えていなかった**(設定の新規導入とツールのメジャーバージョン更新を同時に行うと切り分けが難しくなるため、追随はRenovateへ委ねた)が、その後RenovateのPRで ruff `^0.16.0` / mypy `^2.0.0` へ上がり、**現在は兄弟リポジトリと同じバージョン帯に揃っている**。
- **テストは fakeredis で実Redisを模す**(`app/tests/`): `conftest.py` の `fake_redis` フィクスチャがテストごとに独立したインメモリRedisを返す。`download_queue` は検証対象がハッシュ・リスト・`blpop` といったRedisのデータ構造そのものなので、mockで「hsetをこの引数で呼んだか」を見るのではなく、**実際にコマンドを実行して結果の状態でアサートする**。一方 `RedisConnector` は「プールを正しいパラメータで1度だけ作るか」が検証対象でRedisの振る舞いは関係ないため、兄弟リポジトリと同じく `unittest.mock` で `redis.ConnectionPool` をパッチしている。
  ...
  - **`httpx` をdev依存に入れているのはテスト専用**: `fastapi.testclient`(中身は `starlette.testclient`)がhttpxを要求するが、fastapiはhttpxを本体の依存に含めないため、明示的に足さないと `tests/test_main.py` がコレクション時点で `RuntimeError` になる。**なお starlette 1.x 以降は httpx ではなく httpx2 を推奨する非推奨警告を出す**ため、Renovateが starlette を上げてきたタイミングで httpx2 への切り替えを検討すること(fastapi 0.141時点では starlette 0.52系に解決されるため、まだ不要)。
  - **ローカルで検証するときは必ずロックから環境を作ること**(`poetry install`)。手元の仮想環境へ都度 `pip install` するとロックより新しいバージョンが入り、CIと違う結果になる。実際に「手元では56件パスするのにCIでは `httpx` 欠落でコレクションエラー」という取りこぼしを起こしている。
